export const translations = {
  pt: {
    header: {
      bookButton: "RESERVAR AGORA"
    },
    hero: {
      subtitle: "Sofisticação e qualidade ao seu dispor.",
      cta: "RESERVAR AGORA",
      promo: "Garanta até 20% de Desconto ao realizar sua reserva pelo nosso site oficial."
    },
    rooms: {
      sectionTitle: "ACOMODAÇÕES",
      mainTitle: "NOSSAS SUÍTES",
      description: "Seis categorias de hospedagem, cuidadosamente estruturadas para atender diferentes perfis com o mesmo padrão de excelência.",
      checkAvailability: "VERIFICAR DISPONIBILIDADE",
      items: {
        2: {
          title: "Suíte Casal",
          description: "Conforto sofisticado com vista privilegiada",
          features: ["Cama de Casal", "Frigobar", "Wi-Fi", "Mesa de Trabalho", "TV a cabo"]
        },
        6: {
          title: "Suíte Duplo Solteiro",
          description: "Perfeito para viagens a trabalho ou com amigos",
          features: ["2 Camas Solteiro", "Frigobar", "Wi-Fi", "Mesa de Trabalho", "TV a cabo"]
        },
        3: {
          title: "Suíte Triplo Casal",
          description: "O ápice da exclusividade e requinte",
          features: ["1 Cama de Casal e 1 de Solteiro", "Frigobar", "Wi-Fi", "Mesa de Trabalho", "TV a cabo"]
        },
        7: {
          title: "Suíte Triplo Solteiro",
          description: "Espaço e conforto para grupos e colegas de trabalho",
          features: ["3 Camas Solteiro", "Frigobar", "Wi-Fi", "Mesa de Trabalho", "TV a cabo"]
        },
        4: {
          title: "Suíte Quádruplo",
          description: "Espaço ideal para família ou pequenos grupos, com conforto completo",
          features: ["1 Cama de Casal e 2 de Solteiro", "Frigobar", "Wi-Fi", "Mesa de Trabalho", "TV a cabo"]
        },
        5: {
          title: "Suíte Adaptável",
          description: "Acessibilidade, praticidade e conforto, com estrutura pensada para PCD",
          features: ["Acessível (PCD)", "Frigobar", "Wi-Fi", "Mesa de Trabalho", "TV a cabo"]
        }
      }
    },
    location: {
      sectionTitle: "LOCALIZAÇÃO ESTRATÉGICA",
      mainTitle: "NO CORAÇÃO DE BRASÍLIA",
      accessTitle: "Bem-vindo à sua melhor escolha.",
      accessDescription: "Localizado no Núcleo Bandeirante, conhecido como a cidade pioneira de Brasília por ter sido palco das primeiras estruturas que deram origem à capital, nosso hotel combina acesso estratégico às principais regiões do Distrito Federal com a serenidade de um bairro tradicional e acolhedor.",
      pointsOfInterestTitle: "PONTOS DE INTERESSE",
      contactLabels: {
        address: "Endereço",
        phone: "Telefone",
        service: "Atendimento",
        serviceHours: "24 horas por dia, 7 dias por semana"
      },
      pointsOfInterest: [
        { name: "Aeroporto JK", distance: "7 km" },
        { name: "Esplanada", distance: "14 km" },
        { name: "Park Shopping", distance: "6 km" },
        { name: "Rodoviária Interestadual", distance: "6 km" }
      ]
    },
    footer: {
      description: "Referência em hospitalidade no Distrito Federal, oferecendo conforto, localização estratégica e padrão elevado de atendimento para quem busca a escolha certa em Brasília.",
      contactTitle: "CONTATO",
      termsPrivacy: "Termos de Uso | Política de Privacidade",
      rightsReserved: "Alfa Plaza Hotel. Todos os direitos reservados.",
      developedBy: "Desenvolvido por"
    },
    reservation: {
      title: "Reserva",
      request: "Solicitação",
      hub: {
        bookNow: "Reservar Agora",
        bookNowDesc: "Individual ou Família",
        corporate: "Reserva Faturada",
        corporateDesc: "Exclusivo para Empresas",
        groups: "Grupos e Eventos",
        groupsDesc: "Solicitar Cotação Especial",
        partnership: "Parceria Comercial",
        partnershipDesc: "Seja um Parceiro Alfa Plaza"
      },
      suiteSelection: {
        titleIndividual: "Escolha sua Suíte",
        titleGroup: "Selecione as categorias do grupo",
        selected: "Selecionado",
        nextStep: "PRÓXIMO PASSO"
      },
      dates: {
        checkIn: "Data de Check-in",
        checkOut: "Data de Check-out",
        nextStep: "PRÓXIMO PASSO"
      },
      guests: {
        titleGroup: "Quantas pessoas no grupo?",
        titleIndividual: "Quantos adultos?",
        adult1: "1 Adulto",
        adults2: "2 Adultos",
        adults3: "3 Adultos",
        adults4: "4 Adultos",
        adults5Plus: "5+ Adultos",
        hasChildren: "Viajando com crianças?",
        childrenCourtesy: "Crianças até 5 anos ficam por cortesia.",
        childrenCount: "Quantas crianças?",
        allOver5: "Todos acima de 5 anos?",
        oneChildOver5: "Tem mais que 5 anos?",
        childrenWarning: "*Crianças acima de 5 anos possuem tarifa diferenciada.",
        nextStep: "PRÓXIMO PASSO"
      },
      arrivalTime: {
        title: "Horário de Chegada",
        subtitle: "Previsão estimada para sua recepção",
        almostThere: "QUASE LÁ"
      },
      contact: {
        title: "Dados de Contato",
        subtitle: "Finalize para enviar sua reserva",
        nameLabel: "Nome Completo",
        namePlaceholder: "Qual seu nome?",
        emailLabel: "E-mail",
        emailPlaceholder: "seu@email.com",
        whatsappLabel: "WhatsApp",
        whatsappPlaceholder: "(00) 00000-0000",
        disclaimer: "Ao clicar em finalizar, você será redirecionado para o WhatsApp do nosso setor de reservas para validação final. Quando abrir o WhatsApp, basta enviar a mensagem gerada automaticamente para o número do hotel.",
        finish: "FINALIZAR E ENVIAR"
      },
      rules: {
        corporateTitle: "Diretrizes Corporativas",
        partnershipTitle: "Expansão Comercial",
        corporate: [
          "• A análise de aprovação de crédito está condicionada à existência de uma pré-reserva ativa no sistema.",
          "• Faturamento exclusivo para empresas com cadastro corporativo aprovado.",
          "• Prazo de pagamento: 15 ou 30 dias após a data de check-out.",
          "• Necessário envio de autorização formal em papel timbrado e cópia do Cartão CNPJ atualizado.",
          "• É obrigatória a confirmação prévia de garantia de no-show."
        ],
        partnership: [
          "• Condições especiais para agências e operadoras.",
          "• Acordos de volume para empresas locais.",
          "• Gestão dedicada para contas comerciais."
        ],
        sendEmail: "ENVIAR SOLICITAÇÃO POR E-MAIL"
      },
      types: {
        individual: "Reserva",
        corporate: "Reserva Faturada",
        groups: "Grupos e Eventos",
        partnership: "Parceria Comercial"
      }
    }
  },
  en: {
    header: {
      bookButton: "BOOK NOW"
    },
    hero: {
      subtitle: "Sophistication and quality at your disposal.",
      cta: "BOOK NOW",
      promo: "Get up to 20% Discount by booking through our official website."
    },
    rooms: {
      sectionTitle: "ACCOMMODATIONS",
      mainTitle: "OUR SUITES",
      description: "Six categories of accommodation, carefully structured to meet different profiles with the same standard of excellence.",
      checkAvailability: "CHECK AVAILABILITY",
      items: {
        2: {
          title: "Couple Suite",
          description: "Sophisticated comfort with a privileged view",
          features: ["Double Bed", "Minibar", "Wi-Fi", "Work Desk", "Cable TV"]
        },
        6: {
          title: "Double Single Suite",
          description: "Perfect for business trips or friends",
          features: ["2 Single Beds", "Minibar", "Wi-Fi", "Work Desk", "Cable TV"]
        },
        3: {
          title: "Triple Couple Suite",
          description: "The pinnacle of exclusivity and refinement",
          features: ["1 Double Bed and 1 Single Bed", "Minibar", "Wi-Fi", "Work Desk", "Cable TV"]
        },
        7: {
          title: "Triple Single Suite",
          description: "Space and comfort for groups and colleagues",
          features: ["3 Single Beds", "Minibar", "Wi-Fi", "Work Desk", "Cable TV"]
        },
        4: {
          title: "Quadruple Suite",
          description: "Ideal space for family or small groups, with complete comfort",
          features: ["1 Double Bed and 2 Single Beds", "Minibar", "Wi-Fi", "Work Desk", "Cable TV"]
        },
        5: {
          title: "Accessible Suite",
          description: "Accessibility, practicality and comfort, with structure designed for PWD",
          features: ["Accessible (PWD)", "Minibar", "Wi-Fi", "Work Desk", "Cable TV"]
        }
      }
    },
    location: {
      sectionTitle: "STRATEGIC LOCATION",
      mainTitle: "IN THE HEART OF BRASÍLIA",
      accessTitle: "Welcome to your best choice.",
      accessDescription: "Located in Núcleo Bandeirante, known as the pioneer city of Brasília for having hosted the first structures that gave rise to the capital, our hotel combines strategic access to the main regions of the Federal District with the serenity of a traditional and welcoming neighborhood.",
      pointsOfInterestTitle: "POINTS OF INTEREST",
      contactLabels: {
        address: "Address",
        phone: "Phone",
        service: "Service",
        serviceHours: "24 hours a day, 7 days a week"
      },
      pointsOfInterest: [
        { name: "JK Airport", distance: "7 km" },
        { name: "Esplanade", distance: "14 km" },
        { name: "Park Shopping", distance: "6 km" },
        { name: "Interstate Bus Station", distance: "6 km" }
      ]
    },
    footer: {
      description: "Reference in hospitality in the Federal District, offering comfort, strategic location and high standard of service for those seeking the right choice in Brasília.",
      contactTitle: "CONTACT",
      termsPrivacy: "Terms of Use | Privacy Policy",
      rightsReserved: "Alfa Plaza Hotel. All rights reserved.",
      developedBy: "Developed by"
    },
    reservation: {
      title: "Reservation",
      request: "Request",
      hub: {
        bookNow: "Book Now",
        bookNowDesc: "Individual or Family",
        corporate: "Billed Reservation",
        corporateDesc: "Exclusive for Companies",
        groups: "Groups and Events",
        groupsDesc: "Request Special Quote",
        partnership: "Commercial Partnership",
        partnershipDesc: "Be an Alfa Plaza Partner"
      },
      suiteSelection: {
        titleIndividual: "Choose your Suite",
        titleGroup: "Select group categories",
        selected: "Selected",
        nextStep: "NEXT STEP"
      },
      dates: {
        checkIn: "Check-in Date",
        checkOut: "Check-out Date",
        nextStep: "NEXT STEP"
      },
      guests: {
        titleGroup: "How many people in the group?",
        titleIndividual: "How many adults?",
        adult1: "1 Adult",
        adults2: "2 Adults",
        adults3: "3 Adults",
        adults4: "4 Adults",
        adults5Plus: "5+ Adults",
        hasChildren: "Traveling with children?",
        childrenCourtesy: "Children up to 5 years old stay for free.",
        childrenCount: "How many children?",
        allOver5: "All over 5 years old?",
        childrenWarning: "*Children over 5 years old have a different rate.",
        nextStep: "NEXT STEP"
      },
      arrivalTime: {
        title: "Arrival Time",
        subtitle: "Estimated forecast for your reception",
        almostThere: "ALMOST THERE"
      },
      contact: {
        title: "Contact Details",
        subtitle: "Finish to send your reservation",
        nameLabel: "Full Name",
        namePlaceholder: "What is your name?",
        emailLabel: "E-mail",
        emailPlaceholder: "your@email.com",
        whatsappLabel: "WhatsApp",
        whatsappPlaceholder: "(00) 00000-0000",
        disclaimer: "By clicking finish, you will be redirected to our reservation sector WhatsApp for final validation. When WhatsApp opens, simply send the automatically generated message to the hotel number.",
        finish: "FINISH AND SEND"
      },
      rules: {
        corporateTitle: "Corporate Guidelines",
        partnershipTitle: "Commercial Expansion",
        corporate: [
          "• Credit approval analysis is conditioned on the existence of an active pre-reservation in the system.",
          "• Billing exclusive for companies with approved corporate registration.",
          "• Payment term: 15 or 30 days after check-out date.",
          "• Necessary to send formal authorization on letterhead and copy of updated CNPJ Card.",
          "• Prior confirmation of no-show guarantee is mandatory."
        ],
        partnership: [
          "• Special conditions for agencies and operators.",
          "• Volume agreements for local companies.",
          "• Dedicated management for commercial accounts."
        ],
        sendEmail: "SEND REQUEST BY E-MAIL"
      },
      types: {
        individual: "Individual Reservation",
        corporate: "Billed Reservation",
        groups: "Groups and Events",
        partnership: "Commercial Partnership"
      }
    }
  },
  es: {
    header: {
      bookButton: "RESERVAR AHORA"
    },
    hero: {
      subtitle: "Sofisticación y calidad a su disposición.",
      cta: "RESERVAR AHORA",
      promo: "Obtenga hasta un 20% de Descuento reservando a través de nuestro sitio web oficial."
    },
    rooms: {
      sectionTitle: "ALOJAMIENTOS",
      mainTitle: "NUESTRAS SUITES",
      description: "Seis categorías de alojamiento, cuidadosamente estructuradas para atender diferentes perfiles con el mismo estándar de excelencia.",
      checkAvailability: "VERIFICAR DISPONIBILIDAD",
      items: {
        2: {
          title: "Suite Matrimonial",
          description: "Confort sofisticado con vista privilegiada",
          features: ["Cama de Matrimonio", "Frigobar", "Wi-Fi", "Escritorio", "TV por cable"]
        },
        6: {
          title: "Suite Doble Soltero",
          description: "Perfecto para viajes de trabajo o con amigos",
          features: ["2 Camas Individuales", "Frigobar", "Wi-Fi", "Escritorio", "TV por cable"]
        },
        3: {
          title: "Suite Triple Matrimonial",
          description: "La cúspide de la exclusividad y el refinamiento",
          features: ["1 Cama de Matrimonio y 1 Individual", "Frigobar", "Wi-Fi", "Escritorio", "TV por cable"]
        },
        7: {
          title: "Suite Triple Soltero",
          description: "Espacio y confort para grupos y colegas de trabajo",
          features: ["3 Camas Individuales", "Frigobar", "Wi-Fi", "Escritorio", "TV por cable"]
        },
        4: {
          title: "Suite Cuádruple",
          description: "Espacio ideal para familia o pequeños grupos, con confort completo",
          features: ["1 Cama de Matrimonio y 2 Individuales", "Frigobar", "Wi-Fi", "Escritorio", "TV por cable"]
        },
        5: {
          title: "Suite Adaptable",
          description: "Accesibilidad, practicidad y confort, con estructura pensada para PMR",
          features: ["Accesible (PMR)", "Frigobar", "Wi-Fi", "Escritorio", "TV por cable"]
        }
      }
    },
    location: {
      sectionTitle: "UBICACIÓN ESTRATÉGICA",
      mainTitle: "EN EL CORAZÓN DE BRASILIA",
      accessTitle: "Bienvenido a su mejor elección.",
      accessDescription: "Ubicado en Núcleo Bandeirante, conocido como la ciudad pionera de Brasilia por haber sido escenario de las primeras estructuras que dieron origen a la capital, nuestro hotel combina acceso estratégico a las principales regiones del Distrito Federal con la serenidad de un barrio tradicional y acogedor.",
      pointsOfInterestTitle: "PUNTOS DE INTERÉS",
      contactLabels: {
        address: "Dirección",
        phone: "Teléfono",
        service: "Atención",
        serviceHours: "24 horas al día, 7 días a la semana"
      },
      pointsOfInterest: [
        { name: "Aeropuerto JK", distance: "7 km" },
        { name: "Explanada", distance: "14 km" },
        { name: "Park Shopping", distance: "6 km" },
        { name: "Estación de Autobuses Interestatal", distance: "6 km" }
      ]
    },
    footer: {
      description: "Referencia en hospitalidad en el Distrito Federal, ofreciendo confort, ubicación estratégica y alto nivel de atención para quienes buscan la elección correcta en Brasilia.",
      contactTitle: "CONTACTO",
      termsPrivacy: "Términos de Uso | Política de Privacidad",
      rightsReserved: "Alfa Plaza Hotel. Todos los derechos reservados.",
      developedBy: "Desarrollado por"
    },
    reservation: {
      title: "Reserva",
      request: "Solicitud",
      hub: {
        bookNow: "Reservar Ahora",
        bookNowDesc: "Individual o Familia",
        corporate: "Reserva Facturada",
        corporateDesc: "Exclusivo para Empresas",
        groups: "Grupos y Eventos",
        groupsDesc: "Solicitar Cotización Especial",
        partnership: "Asociación Comercial",
        partnershipDesc: "Sea un Socio Alfa Plaza"
      },
      suiteSelection: {
        titleIndividual: "Elija su Suite",
        titleGroup: "Seleccione las categorías del grupo",
        selected: "Seleccionado",
        nextStep: "PRÓXIMO PASO"
      },
      dates: {
        checkIn: "Fecha de Check-in",
        checkOut: "Fecha de Check-out",
        nextStep: "PRÓXIMO PASO"
      },
      guests: {
        titleGroup: "¿Cuántas personas en el grupo?",
        titleIndividual: "¿Cuántos adultos?",
        adult1: "1 Adulto",
        adults2: "2 Adultos",
        adults3: "3 Adultos",
        adults4: "4 Adultos",
        adults5Plus: "5+ Adultos",
        hasChildren: "¿Viajando con niños?",
        childrenCourtesy: "Niños hasta 5 años se alojan gratis.",
        childrenCount: "¿Cuántos niños?",
        allOver5: "¿Todos mayores de 5 años?",
        childrenWarning: "*Niños mayores de 5 años tienen tarifa diferenciada.",
        nextStep: "PRÓXIMO PASO"
      },
      arrivalTime: {
        title: "Hora de Llegada",
        subtitle: "Previsión estimada para su recepción",
        almostThere: "CASI LISTO"
      },
      contact: {
        title: "Datos de Contacto",
        subtitle: "Finalice para enviar su reserva",
        nameLabel: "Nombre Completo",
        namePlaceholder: "¿Cuál es su nombre?",
        emailLabel: "E-mail",
        emailPlaceholder: "su@email.com",
        whatsappLabel: "WhatsApp",
        whatsappPlaceholder: "(00) 00000-0000",
        disclaimer: "Al hacer clic en finalizar, será redirigido al WhatsApp de nuestro sector de reservas para validación final. Al abrir WhatsApp, simplemente envíe el mensaje generado automáticamente al número del hotel.",
        finish: "FINALIZAR Y ENVIAR"
      },
      rules: {
        corporateTitle: "Directrices Corporativas",
        partnershipTitle: "Expansión Comercial",
        corporate: [
          "• El análisis de aprobación de crédito está condicionado a la existencia de una pre-reserva activa en el sistema.",
          "• Facturación exclusiva para empresas con registro corporativo aprobado.",
          "• Plazo de pago: 15 o 30 días después de la fecha de check-out.",
          "• Necesario envío de autorización formal en papel con membrete y copia de la Tarjeta CNPJ actualizada.",
          "• Es obligatoria la confirmación previa de garantía de no-show."
        ],
        partnership: [
          "• Condiciones especiales para agencias y operadores.",
          "• Acuerdos de volumen para empresas locales.",
          "• Gestión dedicada para cuentas comerciales."
        ],
        sendEmail: "ENVIAR SOLICITUD POR E-MAIL"
      },
      types: {
        individual: "Reserva Individual",
        corporate: "Reserva Facturada",
        groups: "Grupos y Eventos",
        partnership: "Asociación Comercial"
      }
    }
  }
};